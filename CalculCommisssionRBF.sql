USE mas
GO
/****** Object:  StoredProcedure [dbo].[Calcul_Commissiondtc]    Script Date: 28/06/2021 14:26:55 ******/
SET ANSI_NULLS ON
GO
...
--ALTER PROCEDURE [dbo].[Calcul_Commissiondtc]  
--AS 
...
--begin
	...
	inner join GaccExercice 
		...
		where  RegParam_Code='dtc' and  Reg_RegCpt>=0 and  year(reg_dateecheance)>=2020 and reg_banque like '%uib%' -- and (reg_ref is not null or reg_ref1 is not null)  
	...
	-------REMBOURS. ECHEANCE CREDIT-----------------------------------------------------------------------------------------------------------------------
			...
			begin
			insert into BanqueD ( Banque_Code, BanqueD_Date,BanqueD_Libelle, BanqueD_MontantE, BanqueD_MontantD, BanqueD_Statut, BanqueD_Type, BanqueD_Origine, BanqueD_Origine2, BanqueD_OrigineNum, Date_Create, Integ,Banque_Nature,Banque_Formule,BanqueD_Compte,BanqueD_TypeCompt,BanqueD_CompteLib)
			select @Banque_Code,@BanqueD_Date,@BanqueD_Libelle,0,2.975,@BanqueD_Statut,17,5  ,'',@BanqueD_OrigineNum,@Date_Create,0,'TTC','ajouter 2,975 DTT au montant principale lors du remboursement ','','0','REMBOURS. ECHEANCE CREDIT  TTC'
			from Reglement 
			where reg_num =@BanqueD_OrigineNum
			end
	...
	--------Frais Effet et obligation------------------------------------------------------------------------------------------------------------------------
			...
			begin
			insert into BanqueD ( Banque_Code, BanqueD_Date,BanqueD_Libelle, BanqueD_MontantE, BanqueD_MontantD, BanqueD_Statut, BanqueD_Type, BanqueD_Origine, BanqueD_Origine2, BanqueD_OrigineNum, Date_Create, Integ,Banque_Nature,Banque_Formule,BanqueD_Compte,BanqueD_TypeCompt,BanqueD_CompteLib)
			select @Banque_Code,@BanqueD_Date,@BanqueD_Libelle,0,14.875,@BanqueD_Statut,18,5  ,'',@BanqueD_OrigineNum,@Date_Create,0,'TTC','14,875 DT TTC','','0','REGLEM. EFFETS ET OBLIGATIONS'
			from Reglement 
			where reg_num =@BanqueD_OrigineNum
			end	
			if  @RegParam_Code in ('dtc') 
