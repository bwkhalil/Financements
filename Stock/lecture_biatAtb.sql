...
set @Lot = cursor for 
					(
						...
						--use mas
						select case when reg_ref like'f13%' or reg_ref like 'f14%'  then left(reg_ref,7) else left(reg_ref,7) end ,
						case when reg_ref1 like'f13%' or reg_ref1 like 'f14%'  then left(reg_ref1,7) else left(reg_ref1,7) end 
						,reg_num
						from reglement
						inner join GaccExercice 
							on year(reg_datereglement) =GaccExercice.GaccEx_Code 	

						 where RegParam_Code in ('rbf')and reg_num  in ('RF210051') --and  Reg_RegCpt=0 and year(reg_dateecheance)>=2020 and reg_banque like '%biat%' and (reg_ref is not null or reg_ref1 is not null)
					 )
...
select @start,@end,@reg_montant,@ActRnum
...
set @CR = CURSOR LOCAL FAST_FORWARD  for (
	select  date_opr,reference,libelle,debit,credit,num_piece,idllig from (
		...
		where
		 xr.date_opr>=@start 
		and xr.Valider=0 
		and xr.libelle not like '%retard%'  
		and (left(ltrim(rtrim(xr.num_piece)),7) like left(rtrim(ltrim(@ActRef)),7) )or (left(ltrim(rtrim(xr.reference)),7) like left(rtrim(ltrim(@ActRef)),7)) 			
