...
set @Lot = cursor for 
					...
					select * from(
					--use polybat
						select top 100 case when reg_ref like'f13%' or reg_ref like 'f14%'  then right(ltrim(rtrim(reg_ref)),7) else right(ltrim(rtrim(reg_ref)),7) end as ref ,
						case when reg_ref1 like'f13%' or reg_ref1 like 'f14%'  then right(ltrim(rtrim(reg_ref1)),7) else right(ltrim(rtrim(reg_ref1)),7) end as ref1 
					,reg_num as reg_num
						from reglement
						inner join GaccExercice 
							on year(reg_datereglement) =GaccExercice.GaccEx_Code 	

						where   RegParam_Code in ('dtc')  and year(reg_dateecheance)>=2020 and (reg_ref is not null or reg_ref1 is not null) and reg_banque like '%biat%'   /*and  reg_num ='RF20Tr692'*/
...
set @CR = CURSOR LOCAL FAST_FORWARD  for (
	select  date_opr,reference,libelle,debit,credit,num_piece,idllig from (
		...
		/*xr.Valider=0  and*/ xr.libelle not like '%retard%'  and
		 ...
		 (right(ltrim(rtrim(xr.num_piece)),7) like right(rtrim(ltrim(@ActRef)),7) )or (right(ltrim(rtrim(xr.num_piece)),7) like right(rtrim(ltrim(@ActRef1)),7)) 			
				...
				begin	
					...
					from xReleveHistorique xr 
							inner join Reglement 
								...
								and ((rtrim(ltrim(reglement.reg_ref)) like '%'+rtrim(ltrim(xr.num_piece))+'%' or rtrim(ltrim(reglement.reg_ref1)) like '%'+rtrim(ltrim(xr.num_piece))+'%')
							 or
					...
					'Financement DTC'+ ' / -Montant reg- ' +convert(nvarchar(20),(Reglement.Reg_Montant)) +' / du: '+convert(nvarchar(20),(Reglement.Reg_DateReglement),103),
		...
		begin
			...
			begin
				...
				begin
					...
					'Financement DTC'+ ' / -Montant reg- ' +convert(nvarchar(20),(Reglement.Reg_Montant)) +' / du: '+convert(nvarchar(20),(Reglement.Reg_DateReglement),103),	year(date_opr) as libelle,
...
set valider=1,
piece=@ActRnum,rapprocher=@Actrnum
where idllig like @idlg
	...
	begin
		...
		'Financement DTC'+ ' / -Montant reg- ' +convert(nvarchar(20),(@reg_montant)) +' / du: '+convert(nvarchar(20),(@start),103)  as GaccPD_Libelle,
		'' as GaccPD_Coll,
	...
	if (@typeop like '%comm%eche%cre%' and @typeop not like '%tva%') or @typeop like '%tva%'
	begin
		...
		--select '------------',@typeop,@bqmntd,@BanqueD_MontantBanque
		 ...
		 insert into GaccPD (GaccPE_Num,GaccCpt_Num ,GaccPD_Libelle,GaccPD_Debit,GaccPD_Credit,GaccPD_DebitDevise,GaccPD_CreditDevise,GaccPD_Ref,GaccB_RB,gaccpd_jou,GaccPD_Echeance)
			select (select numGce from @gaccnums where type_op='paiement' and id =(select min(id) from @gaccnums where type_op='paiement')),
			case  
				when @typeop like '%inter%' then '65190015'
				when @typeop like '%comm%' then '62799015'
			end
			
			,case  
				when @typeop like '%inter%' then 'ECART INTERET DTC'
	...
	if (@typeop like '%comm%' and @typeop not like '%comm%eche%cre%' and @typeop not like '%tva%')
	begin
	select '*****'
	    select @BanqueD_MontantBanque=@debt+@cred
		insert into GaccPD (GaccPE_Num,GaccCpt_Num ,GaccPD_Libelle,GaccPD_Debit,GaccPD_Credit,GaccPD_DebitDevise,GaccPD_CreditDevise,GaccPD_Ref,GaccB_RB,gaccpd_jou,GaccPD_Echeance)		
		select (select numGce from @gaccnums where type_op='paiement' and id =(select min(id) from @gaccnums where type_op='paiement'))
		,'62700100'  ,rtrim(ltrim(@typeop)), @BanqueD_MontantBanque as debit,0 as credit, @BanqueD_MontantBanque as debitdevise,0 as creditdevise,@Reg_Ref,null,null ,@dateop
		
		union all

		select (select numGce from @gaccnums where type_op='paiement' and id =(select min(id) from @gaccnums where type_op='paiement'))
		,@bq_compte  ,rtrim(ltrim(@typeop)),0 as debit,@BanqueD_MontantBanque as credit,0 as debitdevise,@BanqueD_MontantBanque as creditdevise,
	     @Reg_Ref,'RELEVE '+CAST(RIGHT('00' + CAST(MONTH(@dateop) AS nvarchar(2)), 2) AS varchar(10))+'/'+ CAST(YEAR(@dateop) as nvarchar(4)),
	     @gaccjou_code ,@dateop
