	...
	begin
		...
		begin
			...
			begin
				...
				if @dte_ev2<60 and @reference=@fausse_ref1
				begin
					update @table
					set id=@ptr,touche=3
...
WHILE @@FETCH_STATUS = 0    
	begin
		...
		begin
			...
			if @libelle like '%paiem%princ%' begin select @nep=@ne end
		...
		begin
			...
			select @libelle
				...
				if @libelle like '%Interêts%' begin select @diversys_index=23 end
				...
				if @libelle like '%Interêts%' --and @banqued_montantd>0
				begin
